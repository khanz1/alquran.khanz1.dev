...
import {IVerse, ISurah} from "../interfaces";
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  return {
    props: {
      surah,
      verses: verses.map((verse, i: number) => {
        return {
          ...verse,
...
export default function Surah({ verses, surah }: { verses: IVerse[], surah: ISurah }) {
  ...
  return (
    <Container>
      ...
      {verses.map(verse => <VerseCard key={`${verse.surahNumber}_${verse.verse}`} verse={verse} openCommentary={openCommentary} />)}
    </Container>
  )
}